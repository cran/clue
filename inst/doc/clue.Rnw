...
computing, see e.g.\ \cite{cluster:Hornik:2005} and
...
ensembles.  In Section~2, we describe the underlying data structures,
and the functionality for measuring proximity, obtaining consensus
clusterings, and ``secondary'' clusterings.  Four examples are discussed
...
package~\pkg{stats}, \code{pam()} in recommended package~\pkg{cluster},
and \code{Mclust()} in package~\pkg{mclust}, are available as components
named \code{cluster}, \code{clustering}, and \code{classification},
...
\pkg{clue} provides no support for this ``additional'' structure, and
all computations on partitions are based on their memberships.
...
The hierarchies considered in \pkg{clue} are \emph{total indexed
  hierarchies} and hence correspond in a one-to-one manner to
\emph{ultrametrics} (distances $u_{ij}$ between pairs of objects $i$ and
$j$ which satisfy the ultrametric constraint $u_{ij} = \max(u_{ik},
u_{jk})$ for all triples $i$, $j$, and $k$).
...
added by providing \code{as.hclust()} methods for this class.  In R
2.1.0 or better (again as part of the work on \pkg{clue}),
...
\cite{cluster:DeSoete:1986} to use an SUMT (Sequential Unconstrained
Minimization Technique) approach in turn simplifying the suggestions in
...
the final $u_k$ is the suitably projected onto $\mathcal{U}$.
...
distribution, as well as packages \pkg{cclust}
\citep{cluster:Dimitriadou:2005}, \pkg{cluster}, \pkg{e1071}
\citep{cluster:Dimitriadou+Hornik+Leisch:2005}, and \pkg{mclust} (and of
course, \pkg{clue} itself).
...
\code{c()}, \code{rep()}, and \code{print()}.  Future versions of
\pkg{clue} will add to this list.  E.g., R 2.1.1 will provide a
\code{unique()} method for lists, making it rather straightforward to
provide cluster ensemble methods for finding unique and duplicated
elements, and to tabulate the elements of the ensemble.
...
Resampling the objects will be made possible along with adding basic
infrastructure for dealing with hard prototype-based partitioning (as
for such methods, one can classify the out-of-bag objects to their
closest prototype).  In fact, we believe that for unsupervised learning
methods such as clustering, \emph{reweighting} is conceptually superior
to resampling, and have therefore recently enhanced package~\pkg{e1071}
to provide an implementation of weighted fuzzy $c$-means, and
package~\pkg{flexclust} (not yet released) contains an implementation of
weighted $k$-means by F.~Leisch.  We are currently experimenting with
interfaces for providing ``direct'' support for reweighting via
\code{cl\_boot()}.
...
proximity data are classes~\class{dist} and \class{dissimilarity} in
package~\pkg{cluster} (which basically, but not strictly, extends
\class{dist}), and are both not entirely suited to our needs.  First,
they are confined to \emph{symmetric} dissimilarity data.  Second, they
do not provide enough reflectance.  We also note that the Bioconductor
package~\pkg{graph} contains an efficient subscript method for objects
of class~\class{dist}, but returns a ``raw'' matrix for row/column
subscripting.
...
\code{cl\_dissimilarity()}, returning cluster similarity objects of
classes~\class{cl\_agreement} and \class{cl\_cross\_agreement},
respectively.  Built-in methods for computing dissimilarities may
coincide (in which case they are transforms of each other), but do not
necessarily do so, as there typically are no canonical transformations.
E.g., according to needs and scientific community, agreements might be
transformed to dissimilarities via $d = - \log(s)$ or the square root
thereof \citep[e.g.,]{cluster:Strehl+Ghosh:2003b}, or via $d = 1 - s$.
...
on co-occurences, such as the Katz-Powell
...
\tilde{M}$ which are invariant under permuations of rows and columns,
such as the Normalized Mutual Information (NMI) measure introduced in
...
function~\code{lp.assign()} in package~\pkg{lpSolve}), but primal-dual
...
package~\pkg{optmatch}, available from
\url{http://www.stat.lsa.umich.edu/~bbh/optmatch.html}.  In
...
The partition agreement measures `angle' and `diag' (maximal cosine of
angle between the memberships, and maximal co-classification rate, where
both maxima are taken over all column permutations of the membership
matrices) are based on solving the same LSAP as for Euclidean
dissimilarity.
...
corresponding closest hard partitions, unless the agreement measures are
based on the latter anyways (as currently done for Rand, Katz-Powell,
and NMI).

One could easily add more proximity measures, such as the asymmetric
agreement indices by \cite{cluster:Fowlkes+Mallows:1983a} and the
symmetric variant by \cite{cluster:Wallace:1983}, the ``Variation of
Information'' \citep{cluster:Meila:2003}, and the metric by
\cite{cluster:Mirkin:1996} (which is proportional to one minus the Rand
index).  However, all these measures are rigorously defined for hard
partitions only.  To see why extensions to soft partitions are far from
straightforward, consider e.g.\ measures based on the confusion matrix.
Its entries count the cardinality of certain intersections of sets.
...
\subsubsection{Hierachy Proximities}

Available built-in dissimilarity measures for hierarchies include
\emph{Euclidean} dissimilarity (again, the default measure used by
\code{cl\_dissimilarity()}), which is simply Euclidean dissimilarity
(sum of squared differences) between the associated ultrametrics.
Cophenetic dissimilarity is defined as $1 - c^2$, where $c$ is the
cophenetic correlation coefficient \citep{cluster:Sokal+Rohlf:1962},
i.e., the Pearson product-moment correlation between the ultrametrics.
Finally, gamma dissimilarity is the rate of inversions between the
associated ultrametrics $u$ and $v$ (i.e., the rate of pairs $(i,j)$ and
$(k,l)$ for which $u_{ij} < u_{kl}$ and $v_{ij} > v_{kl}$).  This
measure is a linear transformation of Kruskal's~$\gamma$.  Associated
agreement measures are obtained by suitable transformations of the
dissimilarities~$d$; for Euclidean proximities, we prefer to use $1 / (1
+ d)$ rather than e.g.\ $\exp(-d)$.

One should note that whereas cophenetic and gamma dissimilarities are
invariant to linear transformations, Euclidean ones are not.  Hence, if
only the relative ``structure'' of the dendrograms is of interest,
Euclidean dissimilarities should only be used after transforming the
ultrametrics to a common range of values (e.g., to $[0,1]$).

\subsection{Consensus Clusterings}

There are three main approaches to obtaining consensus clusterings
\citep{cluster:Hornik:2005,cluster:Gordon+Vichi:2001}: in the
...
the problem
\begin{displaymath}
  \sum\nolimits_{b=1}^B w_b d(C, C_b)
  \Rightarrow \min\nolimits_{C \in \mathcal{C}},
\end{displaymath}
i.e., as clusterings~$C^*$ minimizing weighted average dissimilarity.
Analogously, if a similarity measure is given, one can look for
clusterings maximizing weighted average similarity.  Following
\cite{cluster:Gordon+Vichi:1998}, an above $C^*$ is referred to as
(weighted) \emph{median} or \emph{medoid} clustering if the optimum is
sought over the set of all possible base clusterings, or the set $\{
C_1, \ldots, C_B \}$ of the base clusterings, respectively.

If all elements of the ensemble are partitions, package~\pkg{clue}
provides algorithms for computing soft median partitions for weighted
Euclidean and co-membership dissimilarities, respectively.  Let $M_1,
\ldots, M_B$ and $M$ denote the membership matrices of the elements of
the ensemble and their sought median, respectively.  For Euclidean
dissimilarity (ignoring normalization constants), we need to find
\begin{displaymath}
...
suggested in the literature are unfortunately computationally infeasible
for ``typical'' sizes of cluster ensembles ($B \ge 20$, maybe even in
the hundreds).

Package~\pkg{clue} provides two heuristics for (approximately) finding
the soft median for Euclidean dissimilarity.  Method \code{"DWH"} of
function \code{cl\_median()} is an extension of the greedy algorithm in
...
resulting in an ``on-line'' version of method \code{"GV1"}.  This, in
turn, is a fixed-point algorithm for the ``first model'' in
\cite{cluster:Gordon+Vichi:2001}, which iterates between updating $M$ as
...
as well as the sought median have the same number of classes.  The more
general case can be dealt with through suitable projection devices.

When using co-membership dissimilarity, the median is determined by
minimizing
\begin{eqnarray*}
...
\code{cl\_median()} provides a SUMT approach (see Section~\ref{SUMT} on
Page~\pageref{SUMT}) for finding the minimum.  We note that this
...
median \emph{hard} partitions, as e.g.\ done in
...
For ensembles of hierarchies, \code{cl\_median()} provides a built-in
method for approximately minimizing average weighted Euclidean
dissimilarity
\begin{displaymath}
...
sought weighted Euclidean median hierarchy.
...
Function \code{cl\_median()} has synopsis \code{cl\_median(x, method =
  NULL, weights = 1, control = list())}, which allows (similar to the
functions for computing cluster proximities, see Section~\ref{synopsis}
on Page~\pageref{synopsis}) argument \code{method} to be a character
string specifying one of the built-in methods as discussed above, or a
function to be taken as a user-defined method (taking an ensemble, the
case weights, and a list of control parameters as its arguments), again
making it reasonably straightforward to add methods.

Finally, function \code{cl\_medoid()} can be used for obtaining medoid
partitions (using, in principle, arbitrary dissimilarities).  Mudulo
possible differences in the case of ties, this gives the same results as
(the medoid obtained by) \code{pam()} in package~\pkg{cluster}.

Alternative recent approaches to obtaining consensus partitions include
``Bagged Clustering'' \citep[implemented in
...
data set and bootstrap samples from it, respectively.)  Finally, the
approach of \cite{cluster:Fern+Brodley:2004} solves an LSAP for an
asymmetric cost matrix based on object-by-all-classes incidences.
...
using \code{cl\_median()} or \code{cl\_medoid()}.

For the case of Euclidean dissimilarities~$d$ between clusterings,
package~\pkg{clue} additionally provides \code{cl\_pclust()} for direct
prototype-based partitioning based on minimizing the criterion function
$\sum u_{bj}^m d(x_b, p_j)$, the sum of the membership-weighted
Euclidean dissimilarities between the elements~$x_b$ of the ensemble and
the prototypes~$p_j$.  (The underlying feature spaces are that of
membership matrices and ultrametrics, respectively, for partitions and
hierarchies.)
...
prototypes, and computing new prototypes as the class medians.  This may
result in degenerate solutions, and will be replaced by a Hartigan-Wong
\citep{cluster:Hartigan+Wong:1979} style algorithm eventually.  For $m >
1$, a generalization of the fuzzy $c$-means recipe
\citep[e.g.,]{cluster:Bezdek:1981} is used, which alternates between
computing optimal memberships for fixed prototypes, and computing new
prototypes as the class medians.

This procedure is repeated until convergence occurs, or the maximal
number of iterations is reached.  Class medians are computed using (one
of the methods provided by) \code{cl\_median()}. 
...
function~\code{mlbench.cassini()} in package~\pkg{mlbench}
\citep{cluster:Leisch+Dimitriadou:2005}.  Package~\pkg{clue} contains
the data sets \code{Cassini} and \code{CKME}, which are an instance of a
1000-point Cassini data set, and a cluster ensemble of 50 $k$-means
partitions of the data set into three classes, respectively.
...
non-convex structure.  For the median of the ensemble, we obtain
<<>>=
set.seed(1)
m2 <- cl_median(CKME)
@ 
...
<<>>=
table(Median = cl_class_ids(m2), "True Classes" = Cassini$classes)
@ 
(see Figure~\ref{fig:Cassini-median}),
\begin{figure}
  \centering
<<fig=TRUE,echo=FALSE>>=
plot(Cassini$x, col = cl_class_ids(m2), xlab = "", ylab = "")
@ 
  \caption{Median of the Cassini $k$-means ensemble.}
  \label{fig:Cassini-median}
\end{figure}
which has drastically improved performance, and perfect recovery of the
two outer shapes.  In fact,
...
\subsection{Vichi-Gordon macroeconomic data}
...
Table 4 in \cite{cluster:Gordon+Vichi:2001} gives 2-class consensus
...
\code{Macro} and \code{Macro\_Consensus}, respectively.  We compare the
results of \cite{cluster:Gordon+Vichi:2001} using Euclidean
dissimilarities (model 1) to ours as obtained by \code{cl\_median()} with
method \code{"GV1"}.
...
@ 
(Here, we set the random seed for reproducibility.  Of course, one
should really use several replicates of the median heuristic.)

This results in a soft partition with average Euclidean dissimilarity
(the criterion function to be optimized by the median) of
<<>>=
mean(cl_dissimilarity(Macro, m1))
@ 
...
mean(cl_dissimilarity(Macro, m2))
table(CLUE = cl_class_ids(m1), GV2001 = cl_class_ids(m2))
@ 
Interestingly, we are able to obtain a better solution, which is
markedly different from the one reported in the literature, and results
in different classifications for the following countries:
<<>>=
rownames(m2)[cl_class_ids(m1) != cl_class_ids(m2)]
@ 
...
\code{cl\_median()} with method \code{"GV3"}.
...
dissimilarity (the criterion function to be optimized by the median) of
<<>>=
mean(cl_dissimilarity(Kinship82, m1, "comem"))
@ 
...
mean(cl_dissimilarity(Kinship82, m2, "comem"))
@ 
...
@ %
This gives an ultrametric~$u$ for which plotting the corresponding
dendrogram (Figure~\ref{fig:Phonemes}) ``basically'' reproduces Figure~1
...
round(sum((d - u) ^ 2), 4)
@ %
to $d$.  (Note that we currently cannot use \code{cl\_dissimilarity()}
here, as $d$ does not correspond to a hierarchy.)  For the
\code{hclust()} results, we get
<<>>=
...
sapply(hens, function(h) round(sum((d - cl_ultrametric(h)) ^ 2), 4))
@ %
which all exhibit greater Euclidean dissimilarity to $d$ than $u$.  We
can also compare the ``structure'' of the different hierarchies, e.g.\
by looking at the rate of inversions between them:
<<>>=
...
\begin{itemize}
 \item Add data structures for hard/soft partitions based on
  prototypes.
 \item Provide mechanisms to generate cluster ensembles based on
  resampling (assuming prototype-based base partitional clusterers) or
 ...
 \item Add heuristics for finding least squares fits based on iterative
  ...
  \cite{cluster:Hubert+Arabie+Meulman:2004} and the accompanying MATLAB
 ...
 \item Provide heuristics for obtaining \emph{hard} median partitions.
 \item Add facilities for tuning hyper-parameters (most prominently, the
  ...
  \cite{cluster:Roth+Lange+Braun:2002} and
