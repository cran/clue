...
function \code{cl\_consensus()} which has synopsis
\code{cl\_consensus(x, method = NULL, weights = 1, control = list())}.
This allows (similar to the functions for computing cluster proximities,
see Section~\ref{synopsis} on Page~\pageref{synopsis}) argument
...
partitions for weighted Euclidean and co-membership dissimilarities,
respectively.  Let $M_1, \ldots, M_B$ and $M$ denote the membership
...
method for approximately minimizing average weighted squared Euclidean
dissimilarity
\begin{displaymath}
...
For at least Euclidean dissimilarities~$d$ between clusterings,
...
$\sum u_{bj}^m d(x_b, p_j)^2$, the sum of the membership-weighted
squared dissimilarities between the elements~$x_b$ of the ensemble and
...
prototypes, and computing new prototypes as the least squares consensus
clusterings of the classes.  This may result in degenerate solutions,
and will be replaced by a Hartigan-Wong
...
number of iterations is reached.  Least squares consensus clusterings
are computed using (one of the methods provided by)
\code{cl\_consensus()}.
...
@ 
and details as displayed in Figure~\ref{fig:Cassini-mean}:
...
dissimilarities (model 1) to ours as obtained by \code{cl\_consensus()}
with method \code{"GV1"}.
...
should really use several replicates of the consensus heuristic.)
