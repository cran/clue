...
\citep{cluster:Charon+Denoeud+Guenoche:2005}, which is the minimum
...
the form $\sum u_{bj}^m d(x_b, p_j)^e$, the sum of the
...
the classes.  This may result in degenerate solutions, and will be
replaced by a Hartigan-Wong \citep{cluster:Hartigan+Wong:1979} style
algorithm eventually.  For $m > 1$, a generalization of the fuzzy
$c$-means recipe \citep[e.g.,][]{cluster:Bezdek:1981} is used, which
alternates between computing optimal memberships for fixed prototypes,
and computing new prototypes as the consensus clusterings for the
classes.
