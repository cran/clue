...
\pkg{e1071}~\citep{cluster:Dimitriadou+Hornik+Leisch:2005}, and
...
package~\pkg{flexclust}~\citep[not yet released]{cluster:Leisch:2005}
...
invariant under permuations of rows and columns, such as the Normalized
...
For computing median clusterings, package~\pkg{clue} provides function
\code{cl\_median()} which has synopsis \code{cl\_median(x, method =
  NULL, weights = 1, control = list())}.  This allows (similar to the
...
string specifying one of the built-in methods discussed below, or a
...
Function~\code{cl\_medoid()} can be used for obtaining medoid partitions
(using, in principle, arbitrary dissimilarities).  Modulo possible
differences in the case of ties, this gives the same results as (the
medoid obtained by) \code{pam()} in package~\pkg{cluster}.
...
@ 
and details as displayed in Figure~\ref{fig:Cassini-median}:
<<Cassini-median,eval=FALSE>>=
...
\end{figure}
This has drastically improved performance, and perfect recovery of the
